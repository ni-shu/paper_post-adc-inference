...
class InverseCDFNaNError(Exception):
    """Exception raised when NaN/Inf is encountered in inverse CDF bracketing."""
    ...
    def _inverse_cdf(self, p: float, result: SelectiveInferenceResult) -> float:
        ...
        while True:
            ...
            if not np.isfinite(val_a) or not np.isfinite(val_b):
                raise InverseCDFNaNError(
                    "InverseCDFNaNError: NaN/Inf encountered in inverse CDF bracketing: "
                    f"p={p}, a={a}, b={b}, val_a={val_a}, val_b={val_b}, scale={scale}"
                )
...
def _randomized_ppf(
    ...
    while True:
        ...
        if not np.isfinite(val_a) or not np.isfinite(val_b):
            raise InverseCDFNaNError(
                "InverseCDFNaNError: NaN/Inf encountered in randomized PPF bracketing: "
                f"q={q}, a={a}, b={b}, val_a={val_a}, val_b={val_b}, stat={stat}, sigma={sigma}, tau={tau}"
            )
